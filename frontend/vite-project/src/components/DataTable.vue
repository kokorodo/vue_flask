                    ...
                    <template v-else>
                        <template v-for="(e_data, index) in dataFinal" :key="index">
                            <tr v-if="is_ssp_mode || ((index+1) >= currentItemPosition.start && index < currentItemPosition.end)">
                                <td v-for="(e_col, index2) in columnsFinal" class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 align-middle" :class="e_col.class" :key="index2">
